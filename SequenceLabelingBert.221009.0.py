...
def get_aligned_dataset(args, preds, labels, props, word_pos, ku_pos, modes, indexers):
    ...
    for merge_item, ignore_label in zip(merge_items, ignore_labels):
        batched_new_items = []
        for items in batched_merged_items[merge_item]:
            new_items = []
            for item in items:
                item = set(item)
                if len(item) == 0:
...
def get_properties(mode):
    if mode == 'rinna-gpt2':
        return 'rinna/japanese-gpt2-medium', '../results/jpasa.rinna-japanese-gpt2-medium.sl', 30
    elif mode == 'tohoku-bert':
        return 'cl-tohoku/bert-base-japanese-whole-word-masking', '../results/jpasa.bert-base-japanese-whole-word-masking.sl', 30
    elif mode == 'mbart':
        return 'facebook/mbart-large-cc25', '../results/jpasa.mbart-large-cc25.sl', 30
    elif mode == 't5-base':
        return 'megagonlabs/t5-base-japanese-web', '../results/jpasa.t5-base-japanese-web.sl', 30
    elif mode =='rinna-roberta':
        return 'rinna/japanese-roberta-base', '../results/jpasa.rinna-japanese-roberta-base.sl', 30
    elif mode == 'nlp-waseda-roberta-base-japanese':
        return 'nlp-waseda/roberta-base-japanese', '../results/jpasa.nlp-waseda-roberta-base-japanese.sl', 30
    elif mode == 'nlp-waseda-roberta-large-japanese':
        return 'nlp-waseda/roberta-large-japanese', '../results/jpasa.nlp-waseda-roberta-large-japanese.sl', 30
    elif mode == 'rinna-japanese-gpt-1b':
        return 'rinna/japanese-gpt-1b', '../results/jpasa.rinna-japanese-gpt-1b.sl', 30
    elif mode == 'xlm-roberta-large':
        return 'xlm-roberta-large', '../results/jpasa.xlm-roberta-large.sl', 30
    elif mode == 'xlm-roberta-base':
        return 'xlm-roberta-base', '../results/jpasa.xlm-roberta-base.sl', 30
...
def train(run_mode):
    ...
    decoder = allocate_data_to_device(SequenceLabeling(hparams=hyper_parameters), DEVICE)
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=PAD_ID_LABEL)
    ...
    for e in range(NUM_EPOCHS):
        ...
        for items in train_dataloader:
            ...
            loss = criterion(x['logits'].view(-1, decoder.label_size), input_items['labels'].view(-1))
            loss.backward()
        ...
        with torch.inference_mode():
            ...
            for items in dev_dataloader:
                ...
                _, prediction = torch.max(x['logits'], 2)
                tp, fp, fn = get_pr_numbers2(prediction.tolist(), items['labels'].tolist(), items['properties'])
